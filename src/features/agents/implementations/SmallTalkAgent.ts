...
export class SmallTalkAgent implements BaseAgent {
  ...
  constructor(options: SmallTalkOptions = {}) {
    // Utilise maintenant le proxy sécurisé - pas besoin de clé API côté client
    this.options = {
      maxTokens: options.maxTokens || 150,
      temperature: options.temperature || 0.7,
      model: options.model || 'gpt-4o-mini' // Modèle par défaut économique
    };
  }
  ...
  public async execute(props: AgentExecuteProps): Promise<AgentExecuteResult> {
    try {
      ...
      const response = await processSmallTalk(text, {
        ...this.options,
        model: props.model as string || this.options.model
      });
