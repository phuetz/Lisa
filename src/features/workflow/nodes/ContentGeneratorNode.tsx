      ...
      <CardContent>
        ...
        <FormControl fullWidth sx={{ mb: 2 }}>
          ...
          <Select
            ...
            value={data.action}
            label="Action"
            onChange={(evt: SelectChangeEvent<ContentGeneratorNodeData['action']>) => {
              const value = evt.target.value as ContentGeneratorNodeData['action'];
              data.action = value;
            }}
          >
        ...
        {data.action === 'translate' && (
          <TextField
            ...
            onChange={(evt) => {
              data.targetLanguage = evt.target.value;
            }}
            sx={{ mb: 2 }}
          />
        )}
        ...
        {(data.action === 'generate' || data.action === 'rewrite' || data.action === 'draft_email' || data.action === 'draft_message') && (
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select
              ...
              label="Style"
              onChange={(evt: SelectChangeEvent<ContentGeneratorNodeData['style'] | undefined>) => {
                const value = evt.target.value as ContentGeneratorNodeData['style'];
                data.style = value;
              }}
            >
        ...
        {(data.action === 'summarize' || data.action === 'generate') && (
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select
              ...
              label="Longueur"
              onChange={(evt: SelectChangeEvent<ContentGeneratorNodeData['length'] | undefined>) => {
                const value = evt.target.value as ContentGeneratorNodeData['length'];
                data.length = value;
              }}
            >
        ...
        {data.action === 'draft_email' && (
          <>
            ...
            <TextField
              ...
              onChange={(evt) => {
                data.recipient = evt.target.value;
              }}
              sx={{ mb: 2 }}
            />
            <TextField
