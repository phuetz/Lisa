      ...
      <CardContent>
        ...
        <FormControl fullWidth sx={{ mb: 2 }}>
          ...
          <Select
            ...
            value={data.action}
            label="Action"
            onChange={(evt: SelectChangeEvent<MemoryNodeData['action']>) => {
              const value = evt.target.value as MemoryNodeData['action'];
              data.action = value;
            }}
          >
          ...
          <>
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              ...
              <Select
                ...
                label="Type"
                onChange={(evt: SelectChangeEvent<MemoryNodeData['type'] | undefined>) => {
                  const value = evt.target.value as MemoryNodeData['type'];
                  data.type = value;
                }}
              >
            ...
            onChange={(evt) => {
              try {
                if (data.action === 'retrieve') {
                  data.query = JSON.parse(evt.target.value) as Record<string, unknown>;
                } else {
