      ...
      <CardContent>
        ...
        <FormControl fullWidth sx={{ mb: 2 }}>
          ...
          <Select
            ...
            value={data.action}
            label="Action"
            onChange={(evt: SelectChangeEvent<PersonalizationNodeData['action']>) => {
              const value = evt.target.value as PersonalizationNodeData['action'];
              data.action = value;
            }}
          >
        ...
        {(data.action === 'get_recommendations' || data.action === 'adapt_response') && (
          <TextField
            ...
            onChange={(evt) => {
              try {
                data.context = JSON.parse(evt.target.value) as Record<string, unknown>;
              } catch {
                data.context = null;
              }
            }}
