...
import type { ExecutionOptions, ExecutionResult, NodeExecutionResult } from './types/workflow.d';
import { agentRegistry } from '../agents/core/registry';
import { useAppStore, type NodeExecutionStatus, type EdgeExecutionStatus } from '../../store/appStore';

/** Convert error to string message */
function getErrorMessage(error: string | Error | undefined, fallback: string): string {
...
interface NodeExecutionDataRecord {
  input: WorkflowNodeData;
  output: unknown;
  error?: string;
}

type NodeExecutionResultsMap = Record<string, NodeExecutionResult>;

type TransformFunction = (data: unknown, context: ExecutionContext) => unknown;

interface TrackedNodeContext {
  id: string;
  type: string;
...
interface ExecutionContext {
  ...
  nodes: Record<string, TrackedNodeContext>;
  cache: Map<string, unknown>;
  nodeExecutionData: Record<string, NodeExecutionDataRecord>;
}
...
export class WorkflowExecutor {
  ...
  private setNodeExecutionStatus: (nodeId: string, status: NodeExecutionStatus) => void;
  private setEdgeExecutionStatus: (edgeId: string, status: EdgeExecutionStatus) => void;
  ...
  constructor(nodes: Node[], edges: Edge[], options: ExecutionOptions = {}) {
    ...
    this.setNodeExecutionStatus = store.setNodeExecutionStatus;
    this.setEdgeExecutionStatus = store.setEdgeExecutionStatus;
    this.resetExecutionStatus = store.resetExecutionStatus;

    // Initialisation du contexte
    this.initializeContext();
  }

  private updateNodeStatus(nodeId: string, status: NodeExecutionStatus): void {
    this.setNodeExecutionStatus(nodeId, status);
  }

  private asNodeData(value: unknown): WorkflowNodeData {
    if (value && typeof value === 'object') {
      return value as WorkflowNodeData;
    }

    return { value } satisfies WorkflowNodeData;
  }
  ...
  private initializeContext(): void {
    ...
    this.nodes.forEach(node => {
      this.context.nodes[node.id] = {
        ...
        data: (node.data ?? Object.create(null)) as WorkflowNodeData,
      };
    });
  }
  ...
  private log(level: string, message: string, data: Record<string, unknown> = {}): void {
    if (this.debugMode) {
      switch (level.toLowerCase()) {
        case 'debug': logger.debug(message, undefined, data); break;
        case 'info': logger.info(message, undefined, data); break;
        case 'warn': logger.warn(message, undefined, data); break;
        case 'error': logger.error(message, undefined, data); break;
        default: logger.info(message, undefined, data);
      }
    }
  }
  ...
  async execute(): Promise<ExecutionResult> {
    ...
    const nodeResults: NodeExecutionResultsMap = {};
    ...
    try {
      ...
      this.nodes.forEach(node => {
        if (!nodeResults[node.id] && !errors[node.id]) {
          this.updateNodeStatus(node.id, 'skipped');
        }
      });
  ...
  private async executeNodeAndSuccessors(
    node: Node,
    inputData: WorkflowNodeData,
    nodeResults: NodeExecutionResultsMap,
    errors: Record<string, string | Error>
  ): Promise<void> {
    try {
      // Vérifier si le nœud a déjà été exécuté avec succès
      if (nodeResults[node.id]?.success && !errors[node.id]) {
        this.log('info', `Node ${node.id} already executed successfully, skipping re-execution`, { type: node.type });
        return;
      }
      ...
      try {
        ...
        for (const edge of outgoingEdges) {
          ...
          this.setEdgeExecutionStatus(edge.id, 'active');

          // Préparer les données d'entrée pour le nœud suivant
          const targetInput = this.prepareInputForTarget(result, edge);
        ...
        this.context.nodeExecutionData[node.id] = { input: inputData, output: undefined, error: errorMessage };
   ...
   */
  private prepareInputForTarget(sourceResult: unknown, edge: Edge): WorkflowNodeData {
    // Si l'arête a une transformation définie, l'appliquer
    if (edge.data?.transform) {
      try {
        const transformed = this.applyTransform(sourceResult, edge.data.transform);
        return this.asNodeData(transformed);
      } catch (error) {
        this.log('warn', `Transform error on edge ${edge.id}`, { error });
      }
    }

    return this.asNodeData(sourceResult);
  }
   ...
   */
  private applyTransform(data: unknown, transform: unknown): unknown {
    if (typeof transform === 'function') {
      return (transform as TransformFunction)(data, this.context);
    }
    ...
    if (typeof transform === 'string') {
      ...
      transformedString = transformedString.replace(regex, (match, nodeId, outputKey) => {
        const nodeExecution = this.context.nodeExecutionData[nodeId];
        if (nodeExecution && nodeExecution.output && typeof nodeExecution.output === 'object' && outputKey in (nodeExecution.output as Record<string, unknown>)) {
          const value = (nodeExecution.output as Record<string, unknown>)[outputKey];
          return JSON.stringify(value);
        } else {
          this.log('warn', `Expression not resolved: ${match}. Node ${nodeId} or key ${outputKey} not found.`, { nodeId, outputKey, nodeOutput: this.context.nodeExecutionData[nodeId]?.output });
   ...
   * @returns Résultat d'exécution
   */
  private async executeSandboxedNode(node: Node, inputData: WorkflowNodeData): Promise<unknown> {
    ...
    const config = (node.data ?? Object.create(null)) as WorkflowNodeData;
      ...
      switch (type) {
        case 'webhook': {
          ...
          if (!webhookResult.success) {
            throw new Error(getErrorMessage(webhookResult.error, 'Webhook execution failed'));
          }
        ...
        case 'apiCall': {
          ...
          if (!httpResult.success) {
            throw new Error(getErrorMessage(httpResult.error, 'HTTP request failed'));
          }
        ...
        case 'function': {
          ...
          if (!codeResult.success) {
            throw new Error(getErrorMessage(codeResult.error, 'Code execution failed'));
          }
        ...
        case 'transform': {
          ...
          if (!transformResult.success) {
            throw new Error(getErrorMessage(transformResult.error, 'Transform execution failed'));
          }
        ...
        case 'condition': {
          ...
          if (!conditionResult.success) {
            throw new Error(getErrorMessage(conditionResult.error, 'Condition evaluation failed'));
          }
        ...
        case 'delay': {
          ...
          if (!delayResult.success) {
            throw new Error(getErrorMessage(delayResult.error, 'Delay execution failed'));
          }
          ...
          return delayResult.output;
                ...
                case 'set': {
                  const setResult = await agentRegistry.execute('SetAgent', {
                    intent: 'setValue',
                    parameters: {
                      key: typeof config.key === 'string' ? config.key : undefined,
                      value: this.applyTransform(inputData, config.value),
                      input: inputData
                    }
                  });
        
                  if (!setResult.success) {
                    throw new Error(getErrorMessage(setResult.error, 'Set execution failed'));
                  }
        
                  return setResult.output;
                }
                case 'for-each': {
                  const forEachResult = await agentRegistry.execute('ForEachAgent', {
                    intent: 'iterate',
                    parameters: {
                      list: this.applyTransform(inputData, config.listExpression)
                    }
                  });
        
                  if (!forEachResult.success) {
                    throw new Error(getErrorMessage(forEachResult.error, 'For-each execution failed'));
                  }
        
                  return forEachResult.output;
                }
        case 'llmPromptNode': {
          const result = await agentRegistry.execute('LLMAgent', {
            intent: 'prompt',
            parameters: { 
          ...
          if (!result.success) {
            throw new Error(getErrorMessage(result.error, 'LLMAgent execution failed'));
          }
          return { 
            ...inputData, 
        ...
        case 'smallTalkNode': {
          ...
          if (!result.success) throw new Error(getErrorMessage(result.error, 'SmallTalkAgent execution failed'));
        ...
        default: {
          ...
          if (agentRegistry.hasAgent(agentName)) {
            ...
            if (!result.success) {
              throw new Error(getErrorMessage(result.error, `${type} execution failed`));
            }
