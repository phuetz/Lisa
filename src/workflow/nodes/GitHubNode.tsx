...
import { Card, CardContent, Typography, TextField, Box, MenuItem, Select, FormControl, InputLabel, Checkbox, FormControlLabel } from '@mui/material';
      ...
      <CardContent>
        ...
        {(data.action === 'listRepositories' || data.action === 'getRepository' || data.action === 'listIssues' || data.action === 'createIssue' || data.action === 'listPullRequests' || data.action === 'listCommits' || data.action === 'getReadme') && (
          <TextField
            ...
            fullWidth
            value={data.owner || ''}
            onChange={(evt) => (data.owner = evt.target.value)}
            sx={{ mb: 2 }}
          />
        )}
        ...
        {(data.action === 'getRepository' || data.action === 'listIssues' || data.action === 'createIssue' || data.action === 'listPullRequests' || data.action === 'listCommits' || data.action === 'getReadme') && (
          <TextField
            ...
            fullWidth
            value={data.repo || ''}
            onChange={(evt) => (data.repo = evt.target.value)}
            sx={{ mb: 2 }}
          />
        )}
        ...
        {data.action === 'listRepositories' && (
          <TextField
            ...
            fullWidth
            value={data.username || ''}
            onChange={(evt) => (data.username = evt.target.value)}
            sx={{ mb: 2 }}
          />
        )}
        ...
        {(data.action === 'listIssues' || data.action === 'listPullRequests') && (
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select
              ...
              label="État"
              onChange={(evt) => (data.state = evt.target.value as any)}
            >
        ...
        {data.action === 'createIssue' && (
          <>
            <TextField
              ...
              fullWidth
              value={data.title || ''}
              onChange={(evt) => (data.title = evt.target.value)}
              sx={{ mb: 2 }}
            />
            <TextField
              label="Corps"
              ...
              rows={4}
              value={data.body || ''}
              onChange={(evt) => (data.body = evt.target.value)}
              sx={{ mb: 2 }}
            />
            <TextField
              ...
              onChange={(evt) => (data.labels = evt.target.value.split(',').map(s => s.trim()))}
              sx={{ mb: 2 }}
            />
          </>
        )}
        ...
        <TextField
          ...
          type="password"
          value={data.token || ''}
          onChange={(evt) => (data.token = evt.target.value)}
          sx={{ mb: 2 }}
        />
        ...
        <FormControlLabel
          control={
            <Checkbox
              ...
              onChange={(evt) => (data.useCache = evt.target.checked)}
              name="useCache"
              color="primary"
            />
          }
