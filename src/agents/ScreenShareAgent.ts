...
const logger = createLogger('ScreenShareAgent');
...
export class ScreenShareAgent implements BaseAgent {
  ...
  async execute(params: any): Promise<any> {
    ...
    try {
      ...
      logger.error(`Erreur lors de l'exécution de l'action de partage d'écran ${action}:`, error);
  ...
  async getRequiredParameters(task: string): Promise<AgentParameter[]> {
    if (task.includes('partage') && task.includes('écran') && 
      ...
      return [
        { name: 'action', type: 'string', value: 'startScreenShare', required: true },
        { name: 'audio', type: 'boolean', description: 'Inclure l\'audio?', required: false, value: false },
        { name: 'displaySurface', type: 'string', description: 'Type de surface à partager (browser, window, monitor)', required: false, value: 'monitor' }
      ];
    }
    ...
    if (task.includes('partage') && task.includes('écran') && 
      ...
      return [
        { name: 'action', type: 'string', value: 'stopScreenShare', required: true }
      ];
    }
  ...
  private async startScreenShare(options: ScreenShareOptions = {}): Promise<any> {
    ...
    try {
      ...
      stream.getVideoTracks()[0].addEventListener('ended', () => {
        logger.info('Utilisateur a arrêté le partage d\'écran');
        this.stopScreenShare();
      });

      logger.info('Session de partage d\'écran démarrée avec succès');
  ...
  private async stopScreenShare(): Promise<any> {
    ...
    try {
      ...
      logger.info('Session de partage d\'écran arrêtée avec succès');
      ...
      logger.error('Erreur lors de l\'arrêt du partage d\'écran:', error);
  ...
  private async updateScreenShareOptions(options: Partial<ScreenShareOptions>): Promise<any> {
    ...
    try {
      ...
      logger.error('Erreur lors de la mise à jour des options de partage d\'écran:', error);
  ...
  private checkCapabilities(): any {
    const capabilities = {
      ...
      constraints: {
        video: navigator.mediaDevices?.getSupportedConstraints().displaySurface || false,
        audio: navigator.mediaDevices?.getSupportedConstraints().systemAudio || false
      }
    };
