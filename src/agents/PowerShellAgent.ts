...
const logger = createLogger('PowerShellAgent');
...
export class PowerShellAgent implements BaseAgent {
  ...
  async execute(params: any): Promise<any> {
    ...
    try {
      ...
      logger.error(`Erreur lors de l'exécution de l'action PowerShell ${action}:`, error);
  ...
  async getRequiredParameters(task: string): Promise<AgentParameter[]> {
    ...
    if (isCommandExecution) {
      return [
        { name: 'action', type: 'string', value: 'executeCommand', required: true },
    ...
    if (task.includes('système') || task.includes('system') || task.includes('info')) {
      return [
        { name: 'action', type: 'string', value: 'getSystemInfo', required: true }
      ];
    }
  ...
  private async executeCommand(command: string, options: PowerShellOptions = {}): Promise<PowerShellResult> {
    ...
    try {
      ...
      // Mais nous pouvons utiliser un service web ou un backend qui expose cette fonctionnalité
      
      // Simulons l'exécution d'une commande PowerShell (dans une vraie implémentation,
      // nous ferions un appel à un backend ou utiliserions Node.js pour exécuter la commande)
      ...
      const errorMessage = error instanceof Error ? error.message : 'Erreur inconnue';
      logger.error(`Erreur lors de l'exécution de la commande PowerShell: ${command}`, error);
   ...
   */
  private async simulateCommandExecution(command: string, options: PowerShellOptions): Promise<{ output: string; error: string | null; exitCode: number }> {
    ...
    logger.info(`Simulation d'exécution de commande PowerShell: ${command}`);
